...
class Henkel extends DataScan {
    ...
    public function __construct( $default, $website )
    {
        ...
        $this->scan_henkel($website);
    }

    public function scan_henkel($website)
    {
        ...
        for ( $i = 0; $i < 1000; $i+=20 ) {
            ...
            if( empty($results) && $i == 0 ) {
                if($i == 0) {
                    Log::channel('datascan')->info($website->company . ' Request data was empty.');
                }
                break;
            }
